...
trainer.optimizer = torch.optim.SGD(
    param_groups,
    momentum=0.95,  # Increased from 0.9 for smoother training (reduces oscillation)
...
print(f"\n=== SGD3 Optimizer Configuration ===")
print(f"  Main LR: {args.lr_max} (from YAML - optimized for SGD)")
print(f"  Day LR: {args.lr_max_day} (from YAML)")
print(f"  Momentum: 0.95 (increased from 0.9 for smoother training)")
...
print(f"\n=== Changes from SGD2 ===")
print(f"  ✓ Using YAML LR values: {args.lr_max} (3x higher than original AdamW)")
print(f"  ✓ Momentum increased: 0.9 → 0.95 (reduces oscillation)")
print(f"  ✓ Weight decay: {args.weight_decay} (from YAML)")
print(f"  ⚠ If still overfitting, consider uncommenting weight_decay_day increase below")
